...
public class Block {
	...
	public enum DIR4 { // 마우스로 잡은 후의 드래그 방향.
		NONE = -1, // 방향 지정 없음.
		RIGHT, // 오른쪽.
		LEFT, // 왼쪽.
		UP, // 위.
	...
	public static int BLOCK_NUM_X = 6; // 블록을 배치할 수 있는 X 방향 최댓값.
	public static int BLOCK_NUM_Y = 5; // 블록을 배치할 수 있는 Y 방향 최댓값.
}
...
public class BlockControl : MonoBehaviour {
	...
	public float vanish_timer = -0.5f; // 블록이 사라질 때까지의 시간.
	...
	void Start() {
		...
		this.next_step = Block.STEP.READY; // 다음 블록을 대기 중으로.
	}
	...
	void Update() {
		...
		case Block.STEP.FALL:
			// 속도에 중력의 영향을 준다.
			this.fall.velocity += Physics.gravity.y * Time.deltaTime * 1.0f;
			// 세로 방향 위치를 계산.
	...
	public void setColor(Block.COLOR color)
	{
		...
		case Block.COLOR.PINK:
			color_value = new Color(1.0f, 0.0f, 0.5f);
			break;
		case Block.COLOR.SPBLOCK01:
			color_value = new Color(0.5f, 1.0f, 1.0f);
			break;
		case Block.COLOR.SPBLOCK02:
	...
	public void beginRespawn(int start_ipos_y)
	{
		...
		int color_index =
			Random.Range((int)Block.COLOR.FIRST,
			             (int)Block.COLOR.LAST);
		if (Random.Range(0, 100) < 3) {
			this.setColor(Block.COLOR.SPBLOCK01);
		}
